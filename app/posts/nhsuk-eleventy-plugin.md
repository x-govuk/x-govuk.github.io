...
Teams working in the NHS need their own Eleventy plugin, and one that reflected the design idioms common to NHS websites.
...
While the NHS.UK and GOV.UK design systems overlap, there are plenty of areas where they diverge.
...
Yet, apart from layouts, components and styles, the remaining code base was design system agnostic.

So, with our bucket hats on and Oasis blaring in the background, we headed back to the nineties and embraced the third way:

- have the GOV.UK Eleventy Plugin export its utility functions, collections, virtual templates, template formats and Nunjucks filters
- update [`markdown-it-govuk`](https://github.com/x-govuk/markdown-it-govuk) to include a `brand` option, allowing the `nhsuk-*` prefix to be used for classes in the generated HTML

This meant the NHS.UK Eleventy Plugin could inherit much of its underlying code from the GOV.UK Eleventy Plugin, while providing its own Markdown setup, layouts, custom components and configuration options.
...
Creating this new plugin provided an opportunity to revisit earlier design decisions.
...
Version 8 of the GOV.UK Eleventy Plugin changes the way the block quotes get rendered:

- block quotes get styled as quotations, using the design used on GOV.UK, and can now also include an attribution
- alerts get styled using inset text (with the alert type provided as a visually hidden label). The lowest alert level, ‘note’, has the same appearance as that for inset text, while the remaining alert levels have different border and background colours.

The NHS.UK Eleventy Plugin takes a slightly different approach for alerts:
...
We also took this opportunity to review our documentation and how it’s organised. There’s now more detail about to start a new project, while documentation for features and layouts is now split into different sections.
